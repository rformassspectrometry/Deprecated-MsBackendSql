...
be_sql <- backendInitialize(MsBackendSqlDb(), dbcon = conn, files = fls)
sp_sql <- Spectra(backend = be_sql)
sp_sql
```
...
Below, we reproduce and example from the `Spectra` vignette, where
...
```{r}
sp_df <- setBackend(sp_mzr, MsBackendDataFrame())
